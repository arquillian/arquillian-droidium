...
public final class InstrumentationMapper {
     ...
     */
    public static Map<String, InstrumentationConfiguration> map(TestClass testClass) throws InstrumentationMapperException {
        ...
        Method[] methods = testClass.getMethods(Deployment.class);

        // if there is only one @Deployment method without @Instrumentable annotation
        if (methods.length == 1 && methods[0].getAnnotation(Instrumentable.class) == null) {
            InstrumentationConfiguration configuration = new InstrumentationConfiguration();
            configuration.setPort(8080);
            instrumentation.put(getDeploymentName(methods[0]), configuration);
            validate(instrumentation);
            return instrumentation;
        }

        // if there is more than one @Deployment but none of them is @Instrumentable
        // we do not know which one will be instrumented.
        if (methods.length > 1) {
            ...
            if (!instrumentable) {
                throw new InstrumentationMapperException("You have used more than one @Deployment method but without "
                    + "@Instrumentable annotation in all cases. Droidium does not know which deployment you want to "
                    + "instrument. Please annotate at least one @Deployment method with @Instrumentable in order to "
                    + "instrument that @Deployment and try again.");
            }
        }
